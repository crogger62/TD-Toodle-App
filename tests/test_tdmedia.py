...
class DbTests(unittest.TestCase):
    ...
    def test_mark_item_completed_hides_only_active_item(self) -> None:
        conn = sqlite3.connect(":memory:")
        self.addCleanup(conn.close)
        conn.row_factory = sqlite3.Row
        db.ensure_schema(conn)
        db.upsert_items(conn, [db.row_from_task({"id": 1, "title": "Item"}, 10, "now")])

        self.assertTrue(db.mark_item_completed(conn, 1))
        self.assertFalse(db.mark_item_completed(conn, 1))
        row = conn.execute(
            "SELECT completed FROM watch_items WHERE toodledo_id = 1"
        ).fetchone()
...
class SyncTests(unittest.TestCase):
    ...
    @patch("tdmedia.sync.db.connect")
    @patch("tdmedia.sync.tasks.edit_tasks")
    @patch("tdmedia.sync.auth.ensure_tokens")
    def test_completion_updates_local_item_after_toodledo_confirms(
        self, mock_ensure_tokens, mock_edit_tasks, mock_connect
    ) -> None:
        conn = sqlite3.connect(":memory:")
        conn.row_factory = sqlite3.Row
        db.ensure_schema(conn)
        db.upsert_items(
            conn,
            [db.row_from_task({"id": 42, "title": "Preview item"}, 10, "now")],
        )
        mock_connect.return_value = nullcontext(conn)
        mock_ensure_tokens.return_value = {"access_token": "token", "scope": "write"}
        mock_edit_tasks.return_value = [{"id": 42}]

        result = sync.complete_watch_item(42, ":memory:")

        self.assertEqual(result, {"toodledo_id": 42, "title": "Preview item"})
        update = mock_edit_tasks.call_args.args[1][0]
        self.assertEqual(update["id"], 42)
        self.assertIsInstance(update["completed"], int)
        row = conn.execute(
            "SELECT completed FROM watch_items WHERE toodledo_id = 42"
        ).fetchone()
        self.assertEqual(row["completed"], 1)
    ...
    @patch("tdmedia.sync.tasks.edit_tasks")
    @patch("tdmedia.sync.auth.ensure_tokens")
    def test_completion_does_not_change_local_item_when_toodledo_rejects_it(
        self, mock_ensure_tokens, mock_edit_tasks, mock_connect
    ) -> None:
        conn = sqlite3.connect(":memory:")
        conn.row_factory = sqlite3.Row
        ...
        mock_connect.return_value = nullcontext(conn)
        mock_ensure_tokens.return_value = {"access_token": "token", "scope": "write"}
        mock_edit_tasks.return_value = [{"id": 42, "errorCode": 123, "errorDesc": "Nope"}]

        with self.assertRaisesRegex(RuntimeError, "Toodledo completion failed"):
            sync.complete_watch_item(42, ":memory:")
...
class WebTests(unittest.TestCase):
    def test_page_shows_last_successful_sync(self) -> None:
        ...
        self.assertIn('data-toodledo-id="42"', page)
        self.assertIn("No longer needed", page)
        self.assertIn("Mark this task complete in Toodledo?", page)
        self.assertIn("This item will disappear from the active listing after completion.", page)
        self.assertIn("Completion preview", page)
        self.assertIn('action="/complete"', page)
        self.assertIn('name="csrf_token"', page)
        self.assertIn("Complete in Toodledo", page)
        self.assertIn("Updating Toodledo...", page)
        self.assertIn('completeConfirmation.hidden = false', page)
        self.assertIn('classList.add("is-completion-preview")', page)
        self.assertIn('classList.remove("is-completion-preview")', page)
        self.assertIn('completeForm.classList.add("is-completing")', page)
        ...
        self.assertIn("tdmedia v1.2.0", page)
